...
* Color images decoded with OpenCV has axis sequence XY[BGR], whereas `caffe.io.load_image` decodes to axis sequence XY[RGB]. Caffe models trained on ImageNet has channel sequence BGR
...
* Caffe does not support data transformation for HDF5 input. Mean subtraction needs to be done outside. Also, each input image should have axis sequence [BGR]XY, where X, Y $\in$ [0,227]
...
what's done:
	...
	* Trained multiclass binary classifier on 4096 feature responses
what's not done:
