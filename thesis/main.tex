...
\begin{enumerate}
\item As discussed under \hyperref[eqn:pn2]{(\ref{eqn:pn2})}, the difference between the accuracy of stage 1 improvement and the original HEX model is little, except under 90\% relabelling rate, in the extended concept space. This sole improvement is because the sigmoid function diminishes the difference between the responses on concepts, when they are all very low (refer to \hyperref[tab:cnnresp]{table~\ref{tab:cnnresp}}). Note that such improvement of numerical stability does not affect the top-1 accuracy.
\item In the extended state space, stage 2 improvement brings a significant accuracy enhancement for all relabelling rates. Under 50\% relabelling rate, its accuracy leads the original HEX model by 14.49 percentage points. In addition, such improvement is consistent over all labels, as shown in the broken down accuracy in \hyperref[tab:cnn0acc]{table~\ref{tab:cnn0acc}}, \ref{tab:cnn50acc}, and \ref{tab:cnn90acc}. This is a clear sign that the relationships considered in the original HEX model are too weak. Hence, a possible direction of further improvement is to incorporate more relationships.

In the original state space, the accuracy of stage 2 improvement falls significantly below the original HEX model. This ...
\item The accuracy of stage 3 improvement falls slightly behind stage 2 under 0\% relabelling rate, but takes over as the relabelling rate grows. Under 90\% relabelling rate, stage 3 improvement is the only system to achieve accuracy higher than 10\% with the extended state space.
