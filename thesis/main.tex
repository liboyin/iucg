...
With CNN as the underlying unary classifier, the overall performance of different variants of the HEX model are compared in \hyperref[tab:cnnacc]{table~\ref{tab:cnnacc}}. These performances are broken down to labels in \hyperref[tab:cnn0acc]{table~\ref{tab:cnn0acc}}, \ref{tab:cnn50acc}, and \ref{tab:cnn90acc}. \hyperref[tab:cnnresp]{Table~\ref{tab:cnnresp}} contains a summary of response of CNN on validation images, broken down to labels. The unary and pairwise weightings learned in stage 3 are shown in \hyperref[tab:cnnunary]{table~\ref{tab:cnnunary}} and \ref{tab:cnnpairwise}. Observations on these results are listed as follows:
\begin{enumerate}
\item As a confirmation to \hyperref[eqn:pn2]{(\ref{eqn:pn2})}, the difference between the accuracy of stage 1 improvement and the original HEX model is little.
\item In the extended state space, stage 2 improvement brings a significant accuracy enhancement for all relabelling rates. Under 50\% relabelling rate, its accuracy leads the original HEX model by 14.49 percentage points. In addition, such improvement is consistent over all labels, as shown in the broken down accuracy in \hyperref[tab:cnn0acc]{table~\ref{tab:cnn0acc}}, \ref{tab:cnn50acc}, and \ref{tab:cnn90acc}. This is a clear sign that the relationships considered in the original HEX model are too weak. Hence, a potential direction for further improvement is to incorporate more relationships. In the original state space, however, the accuracy of stage 2 improvement falls significantly below the original HEX model under 50\% and 90\% relabelling rate. 
\item The accuracy of stage 3 improvement falls slightly below stage 2 under 0\% relabelling rate, but soon takes over as the relabelling rate grows. Under 90\% relabelling rate, stage 3 improvement is the only system to achieve accuracy higher than 10\% in the extended state space.

It has been noticed that some unary weights are set to zero by optimisation. This corresponds to constant unary contributions on certain nodes. It doesn't indicate zero overall contribution, as their confidence is still encoded in pairwise term of the potential. Concepts with unary weight set to zero received significant accuracy enhancement under high relabelling rate (``bus'', ``bicycle'', ``horse'', ``sheep'', and ``bird''). Under 90\% relabelling rate, all concepts with accuracy greater than 0 received zero unary weight. However, no trend has been found regarding why these nodes receive zero unary weights.
...
\end{tabular}
\caption{Comparison of accuracy broken down to concepts, under 50\% relabelling rate, in the extended state space, with CNN as the underlying classifier. Note that: 1. Stage 2 improvement managed to increase the accuracy by more than 10 percentage points in more than half of bottom-level concepts. 2. The accuracy on abstract concepts are consistent for all models, showing sufficiently high confidence on nodes corresponding to abstract concepts. 3. Stage 2 and 3 have similar overall accuracy, but the broken down accuracy are very different. The accuracy on most concepts dropped, but the accuracy corresponding to nodes whose unary weights are set to 0 received significant accuracy enhancement.}
\label{tab:cnn50acc}
\end{table}
...
\end{tabular}
\caption{Comparison of accuracy broken down to concepts, under 90\% relabelling rate, in the extended state space, with CNN as the underlying classifier. Concepts that received accuracy boost are those with zero unary weights.}
\label{tab:cnn90acc}
\end{table}
...
Scalable until stage 2 with MAP LBP. Stage 3 is not scalable due to the high-order terms in \hyperref[eqn:legal]{(\ref{eqn:legal})}.
